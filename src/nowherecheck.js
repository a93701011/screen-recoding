...
async function collectfile(a){
  const subfolder = `volumn=${dna_list[a][1]}`
  let fn = `${a}_${dna_list[a]}_nowhere.html`
  try{
  await checkFile(`C:/Users/adminkaren/Documents/screenshot/nowhere/Archive/${subfolder}/${fn}`)
  await moveFile(`C:/Users/adminkaren/Documents/screenshot/nowhere/Archive/${subfolder}/${fn}`, `C:/Users/adminkaren/Documents/screenshot/nowhere/html/all/${fn}`)
  }catch{}
}

let count = 0;

async function checkmisingfile(a){
  const subfolder = `volumn=${dna_list[a][1]}`
  let fn = `${a}_${dna_list[a]}_nowhere.html`
  
  try{
  await checkFile(`C:/Users/adminkaren/Documents/screenshot/nowhere/html/all/${fn}`)
  }catch{
  
    await timer(500);
    await screenShot(a)
    console.log(`Task ${a} ${dna_list[a]} done!`);
    count++
  }
